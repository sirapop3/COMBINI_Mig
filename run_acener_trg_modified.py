...
#torch.cuda.set_per_process_memory_fraction(0.5, 0)  # Use only 50% of GPU memory
...
class ACEDatasetNER(Dataset):
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):
            ...
            # words = ["playing", "basketball"]
            # tokens = [["play", "##ing"], ["basket", "##ball"]]            
            # subword2token = [0, 0, 1, 1]
            # token2subword = [0, 2, 4]
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                sentence_trgs = triggers[n]
                
                sentence_ners += sentence_trgs
                ...
                right_length = len(subwords) - doc_sent_end  # len of right context
                sentence_length = doc_sent_end - doc_sent_start
                # The use of half_context_length is a strategy 
                # to include balanced context around the target sentence 
                # while staying within the maximum sequence length limit.                
                ...
                if sentence_length > max_num_subwords:
                    print(
                        f"  Skip sentence due to exceeding subtoken limit: {sentence_length}"
                    )
                    continue
                ...
                # target_tokens = inputs surrounded by left-to-right context
                ...
                for entity_start in range(left_context_length, left_context_length + sentence_length):
                    # doc_entity_start begins from the target setnence
                    ...
                    for entity_end in range(entity_start + 1, left_context_length + sentence_length + 1):
                        ...
                        entity_infos.append(
                            ((entity_start+1, entity_end), # Subword positions (shifted by +1 for [CLS])
                            label, 
                            (subword2token[doc_entity_start], subword2token[doc_entity_end-1])) # Original token positions
                        )
    ...
    def __getitem__(self, idx):
        ...
        if self.model_type not in ['bertspan', 'robertaspan', 'albertspan']:
            if self.model_type.startswith('albert'):
                ...
                input_ids = input_ids \
                    + [self.tokenizer.encode('[unused0]', add_special_tokens=False)[0]] * (len(entry['examples'])) \
                    + [self.tokenizer.pad_token_id] * (self.max_pair_length - len(entry['examples']))   
                input_ids = input_ids \
                    + [self.tokenizer.encode('[unused1]', add_special_tokens=False)[0]] * (len(entry['examples'])) \
                    + [self.tokenizer.pad_token_id] * (self.max_pair_length - len(entry['examples']))                
        ...
        for x_idx, x in enumerate(entry['examples']):
            m1 = x[0]  # entity subtoken position
            label = x[1]
            mentions.append(x[2]) # entity original token position
            ...
            # DiMB-RE having cross-sent: Skip if any position ID is negative
            if m1[0] < 0 or m1[1] < 0:
                print(
                    f"Skipping example at index {x_idx} due to negative position IDs: m1={m1}:: from{entry['example_L']}"
                )
                continue            

            w1 = x_idx
            w2 = w1 + num_pair  # Offset by max_pair_length
            ...
            # For start and end position, both can attend to
            # start and end position, as well as all sentence tokens
            # full_attention_mask: 1-d array
            # attention_mask: 2-d array
            
            # Note: How about allowing attention among marker tokens?
            for xx in [w1, w2]:
                full_attention_mask[xx] = 1  
                for yy in [w1, w2]:
                    attention_mask[xx, yy] = 1  # tie the pair marker
                attention_mask[xx, :L] = 1  # marker CAN attend original tokens
        ...
        # print(
        #     torch.tensor(input_ids).size(),
        #     (len(attention_mask), len(attention_mask[0])),
        #     torch.tensor(position_ids).size(),
        #     torch.tensor(labels, dtype=torch.int64).size(),
        #     torch.tensor(mention_pos).size(),
        #     torch.tensor(full_attention_mask).size()
        # )
        # assert 1==0
...
def train(args, model, tokenizer):
    ...
    if args.do_test:
        ...
        dev_dataset = ACEDatasetNER(tokenizer=tokenizer, args=args, do_test=True)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #     raise ValueError("input_ids should be a 2D tensor.")

            # # Validate label values
            # if (inputs['labels'] < 0).any() or (inputs['labels'] >= 13).any():
            #     print(inputs['labels'])
            #     raise ValueError("Labels are out of range.")
            
            # # Print the position IDs for debugging
            # print("Position IDs:", inputs['position_ids'])
            # # Check for negative values
            # negative_ids = (inputs['position_ids'] < 0).any()
            # if negative_ids:
            #     print("There are negative position IDs.")
            # # Check for values exceeding max position embeddings
            # exceeding_ids = (inputs['position_ids'] >= model.config.max_position_embeddings).any()
            # if exceeding_ids:
            #     print("There are position IDs exceeding the maximum allowed.")
            # # Find and print the maximum position ID
            # max_position_id = inputs['position_ids'].max().item()  # Get the maximum position ID
            # print("Maximum Position ID:", max_position_id)
            # # Raise error if any condition is met
            # if negative_ids or exceeding_ids:
            #     raise ValueError("Position IDs are out of range.")                    
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    # Should be disabled to predict test set!
                    if args.evaluate_during_training:  
                        ...
                        if patience_counter >= max_patience:  # Check if patience exceeded
                            print("Early stopping triggered. No improvement in F1 score for {} evaluations.".format(max_patience))
                            break  # Exit the training loop                            

                    if not args.do_test and update:
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    for example_index, pair_dict in scores.items():  
        ...
        # We don't need to remove nested spans
        ...
        for item in sentence_results:
            ...
            if args.output_results:
                if not pred_ner_label.isupper():
                    predict_ners[example_index].append((m2[0], m2[1], pred_ner_label))
                else:
                    predict_trgs[example_index].append((m2[0], m2[1], pred_ner_label))
            if (example_index, m2, pred_ner_label) in ner_golden_labels:
                cor += 1        
    ...
    if args.output_results:
        ...
        for l_idx, line in enumerate(f):
            ...
            for n in range(num_sents):
                item_ner = predict_ners.get((l_idx, n), [])
                item_ner.sort()
                predicted_ner.append(item_ner)
                item_trg = predict_trgs.get((l_idx, n), [])
                item_trg.sort()
                predicted_trg.append(item_trg)                

            data['predicted_ner'] = predicted_ner
            data['predicted_triggers'] = predicted_trg
...
def main():
    ...
    parser.add_argument('--max_mention_ori_trg_length', type=int, default=8,  help="")
    ...
    if args.do_train and args.local_rank in [-1, 0]:
        create_exp_dir(
            args.output_dir, 
            scripts_to_save=[
                '/home/user/combiniiii/COMBINI_PL-Marker-main/run_acener.py',
                '/home/user/combiniiii/COMBINI_PL-Marker-main/transformers/src/transformers/modeling_bert.py',
                '/home/user/combiniiii/COMBINI_PL-Marker-main/transformers/src/transformers/modeling_albert.py'
            ]
        )
    ...
    args.model_name_or_path = "/home/user/combiniiii/COMBINI_PL-Marker-main/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext"

    args.model_type = args.model_type.lower()

    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]

    # # Load configuration, tokenizer, and model directly from Hugging Face
    ...
    config = config_class.from_pretrained(
        #args.config_name if args.config_name else args.
        "/home/user/combiniiii/COMBINI_PL-Marker-main/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext", num_labels=num_labels
    )
    tokenizer = AutoTokenizer.from_pretrained(
        #args.model_name_or_path
        "/home/user/combiniiii/COMBINI_PL-Marker-main/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext",  do_lower_case=args.do_lower_case
    )
    ...
    model = model_class.from_pretrained(
        #args.model_name_or_path,
        "/home/user/combiniiii/COMBINI_PL-Marker-main/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext",
    ...
    if args.do_train and args.lminit: 
        ...
        # if args.model_type.startswith('albert'):
        #     word_embeddings = model.albert.embeddings.word_embeddings.weight.COMBINI-data
        ...
        #     word_embeddings = model.roberta.embeddings.word_embeddings.weight.COMBINI-data
        #     word_embeddings[50261].copy_(word_embeddings[mask_id])   # entity
        #     word_embeddings[50262].COMBINI-data.copy_(word_embeddings[entity_id])
        # else:
        ...
        objs = tokenizer.encode('[unused0]', add_special_tokens=False)[0]
        obje = tokenizer.encode('[unused1]', add_special_tokens=False)[0]        
        word_embeddings[objs].copy_(word_embeddings[mask_id])      
        word_embeddings[obje].copy_(word_embeddings[entity_id])
    ...
    if args.do_train and \
        ...
        update = True
        if not args.do_test and args.evaluate_during_training:
            results = evaluate(args, model, tokenizer)
            f1 = results['f1']
            if f1 > best_f1:
                best_f1 = f1
