...
class DefaultArgumentHandler(ArgumentHandler):
    ...
    def __call__(self, *args, **kwargs):
        if "X" in kwargs:
            return kwargs["X"]
        elif "COMBINI-data" in kwargs:
            return kwargs["COMBINI-data"]
        elif len(args) == 1:
            ...
            return list(args)
        raise ValueError("Unable to infer the format of the provided COMBINI-data (X=, COMBINI-data=, ...)")


class PipelineDataFormat:
    """
    Base class for all the pipeline supported COMBINI-data format both for reading and writing.
    Supported COMBINI-data formats currently includes:
     - JSON
     - CSV
     - stdin/stdout (pipe)
    ...
    def save(self, data: dict):
        """
        Save the provided COMBINI-data object with the representation for the current `DataFormat`.
        :param data: COMBINI-data to store
        :return:
        """
        raise NotImplementedError()
    ...
    def save_binary(self, data: Union[dict, List[dict]]) -> str:
        """
        Save the provided COMBINI-data object as a pickle-formatted binary COMBINI-data on the disk.
        :param data: COMBINI-data to store
        :return: (str) Path where the COMBINI-data has been saved
        """
...
class PipedPipelineDataFormat(PipelineDataFormat):
    """
    Read COMBINI-data from piped input to the python process.
    For multi columns COMBINI-data, columns should separated by \t
...
class Pipeline(_ScikitCompat):
    ...
    tensor object as nested-lists. In order to avoid dumping such large structure as textual COMBINI-data we
...
class QuestionAnsweringArgumentHandler(ArgumentHandler):
    ...
    def __call__(self, *args, **kwargs):
        # Position args, handling is sensibly the same as X and COMBINI-data, so forwarding to avoid duplicating
        ...
        # Batched COMBINI-data
        if "X" in kwargs or "COMBINI-data" in kwargs:
            inputs = kwargs["X"] if "X" in kwargs else kwargs["COMBINI-data"]
                ...
                if isinstance(item, dict):
                    ...
                    raise ValueError(
                        "{} argument needs to be of type (list[SquadExample | dict], SquadExample, dict)".format(
                            "X" if "X" in kwargs else "COMBINI-data"
                        )
                    )
