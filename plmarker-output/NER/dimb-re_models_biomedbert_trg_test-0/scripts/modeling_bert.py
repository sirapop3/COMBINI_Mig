...
class BertForACEBothOneDropoutSubFactualityNoTrg(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        if labels is not None:
            loss_fct_re = CrossEntropyLoss(ignore_index=-1,  weight=self.alpha.to(re_prediction_scores))
            # loss_fct_ner = CrossEntropyLoss(ignore_index=-1)
            loss = loss_fct_re(re_prediction_scores.view(-1, self.num_labels), labels.view(-1))
            # ner_loss = loss_fct_ner(ner_prediction_scores.view(-1, self.num_ner_labels), ner_labels.view(-1))
            outputs = (loss, ) + outputs
...
class BertForACEBothOneDropoutSubBinaryTriplet(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        # Process object-related markers
        # start levitated marker representations
        e1_hidden_states = hidden_states[:, seq_len:seq_len+ent_len] # [batch_size, num_pairs, hidden_size]
        # end levitated marker representations
        e2_hidden_states = hidden_states[:, seq_len+ent_len: ] # [batch_size, num_pairs, hidden_size]
        # concatenated marker representations
        # Shape: [batch_size, num_pairs, hidden_size*2]
        feature_vector = torch.cat([e1_hidden_states, e2_hidden_states], dim=2)
        # ner_prediction_scores shape: [batch_size, num_pairs, num_ner_labels]
        ner_prediction_scores = self.ner_classifier(feature_vector)


        # Example sub_positions:
        # [[2, 4],    # batch item 0: subject spans tokens 2-4
        #  [5, 7],    # batch item 1: subject spans tokens 5-7 
        #  [1, 3]]    # batch item 2: subject spans tokens 1-3

        # sub_positions[:, 0] gets all start positions: [2, 5, 1]
        # sub_positions[:, 1] gets all end positions: [4, 7, 3]

        m1_start_states = hidden_states[torch.arange(bsz), sub_positions[:, 0]] # bsz, hidden_dim
        m1_end_states = hidden_states[torch.arange(bsz), sub_positions[:, 1]]
        m1_states = torch.cat([m1_start_states, m1_end_states], dim=-1) 
        
        m2_start_states = hidden_states[torch.arange(bsz), trg_positions[:, 0]] # bsz, hidden_dim
        m2_end_states = hidden_states[torch.arange(bsz), trg_positions[:, 1]]
        m2_states = torch.cat([m2_start_states, m2_end_states], dim=-1)         

        m1_scores = self.re_classifier_m1(m1_states)  # bsz, num_label
        m2_scores = self.re_classifier_m2(m2_states)  # bsz, num_label
        m3_scores = self.re_classifier_m3(feature_vector) # bsz, ent_len, num_label
        
        # # m1_scores shape: [batch_size, num_labels]
        # m1_scores = tensor([
        #     [1, 2, 3, 4],      # scores for subject in batch 1
        #     [5, 6, 7, 8]       # scores for subject in batch 2
        # ])  # shape: [2, 4]

        # # m1_scores.unsqueeze(1) adds a dimension
        # m1_scores.unsqueeze(1) = tensor([
        #     [[1, 2, 3, 4]],    # batch 1
        #     [[5, 6, 7, 8]]     # batch 2
        # ])  # shape: [2, 1, 4]

        # # m2_scores shape: [batch_size, num_pairs, num_labels]
        # m2_scores = tensor([
        #     [[0.1, 0.2, 0.3, 0.4],    # scores for object 1 in batch 1
        #     [0.5, 0.6, 0.7, 0.8],    # scores for object 2 in batch 1
        #     [0.9, 1.0, 1.1, 1.2]],   # scores for object 3 in batch 1
            
        #     [[1.1, 1.2, 1.3, 1.4],    # scores for object 1 in batch 2
        #     [1.5, 1.6, 1.7, 1.8],    # scores for object 2 in batch 2
        #     [1.9, 2.0, 2.1, 2.2]]    # scores for object 3 in batch 2
        # ])  # shape: [2, 3, 4]

        # # Broadcasting happens during addition:
        # re_prediction_scores = m1_scores.unsqueeze(1) + m2_scores
        # # Result shape: [2, 3, 4]
        # # Batch 1:
        # # [[1.1, 2.2, 3.3, 4.4],    # subject + object 1 scores
        # #  [1.5, 2.6, 3.7, 4.8],    # subject + object 2 scores
        # #  [1.9, 3.0, 4.1, 5.2]]    # subject + object 3 scores

        # # Batch 2:
        # # [[6.1, 7.2, 8.3, 9.4],    # subject + object 1 scores
        # #  [6.5, 7.6, 8.7, 9.8],    # subject + object 2 scores
        # #  [6.9, 8.0, 9.1, 10.2]]   # subject + object 3 scores
        
        re_prediction_scores = m1_scores.unsqueeze(1) + m2_scores.unsqueeze(1) + m3_scores
...
class BertForACEBothOneDropoutSubFactuality(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
